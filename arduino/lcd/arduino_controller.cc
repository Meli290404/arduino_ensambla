...
void ArduinoController::sendCommand(std::string &command){
    if (isConnected){
        std::string withNewline = command + "\n";  // Agregar salto de línea para señalas que termina la palabra
        write(serialPort, withNewline.c_str(), withNewline.size()); //se cambia a que escriba el string con withNewLine en vez de solo el primer caracter.
    }else {
